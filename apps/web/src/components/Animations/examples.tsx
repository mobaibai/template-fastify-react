...
// 基础动画演示
const BasicAnimationsDemo: React.FC<{ showAnimations: boolean }> = ({
  showAnimations,
}) => {
  return (
...
// 高级动画演示
const AdvancedAnimationsDemo: React.FC<{ showAnimations: boolean }> = ({
  showAnimations,
}) => {
  return (
    <div className="space-y-6">
        ...
        <>
          ...
          <AnimationSlide direction="slideInUp" distance={50} delay={200}>
            <Card title="缩放动画" className="shadow-lg">
              <Row gutter={[16, 16]}>
                {(['scaleIn', 'pulse', 'bounce'] as const).map(
                  (scaleType, index) => (
          ...
          <AnimationSlide direction="slideInLeft" distance={50} delay={1000}>
            <Card title="旋转动画" className="shadow-lg">
              <Row gutter={[16, 16]}>
                {(['rotateIn', 'spin', 'flip'] as const).map(
                  (rotateType, index) => (
...
// 组合动画演示
const CombinationAnimationsDemo: React.FC<{ showAnimations: boolean }> = ({
  showAnimations,
}) => {
  return (
    <div className="space-y-6">
...
// 序列动画演示
const SequenceAnimationsDemo: React.FC<{ showAnimations: boolean }> = ({
  showAnimations,
}) => {
...
// 注意力动画演示
const AttentionAnimationsDemo: React.FC<{ showAnimations: boolean }> = ({
  showAnimations,
}) => {
  return (
    <div className="space-y-6">
...
// 预设动画演示
const PresetsAnimationsDemo: React.FC<{ showAnimations: boolean }> = ({
  showAnimations,
}) => {
  return (
    <div className="space-y-6">
        ...
        <>
              ...
              <Row gutter={[16, 16]}>
                {Object.entries(entranceAnimations)
                  ...
                  .map(([name, config], index) => {
                    // 处理单个效果或效果数组，确保类型正确
                    const effects: import('./AnimationContainer').AnimationEffect[] =
          ...
          <AnimationSlide direction="slideInRight" distance={50} delay={3000}>
            <Card title="序列动画预设" className="shadow-lg">
              <Row gutter={[16, 16]}>
                {Object.entries(sequenceAnimations).map(
                  ([name, steps], index) => (
                    <Col xs={24} md={8} key={name}>
                      <AnimationSequence
                        steps={steps}
                        loop={false}
                        globalDelay={3200 + index * 400}
                      >
